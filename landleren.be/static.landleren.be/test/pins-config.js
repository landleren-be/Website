/* www.landleren.be */
/* pins file for placing pins on the map - deprecated from tryjs version */
var pins_config = {
    "pins": [
        {}
    ]
...
function tryjsAddEvent(id) {
    ...
    if (pins_config.pins[id].active === !0) {
        ...
        if (isTouchEnabled()) {
            ...
            obj.on("touchend", function() { jQuery("#" + id).css({ "fill": pins_config.pins[id].upColor }); if (pins_config.pins[id].target === "new_window") { window.open(pins_config.pins[id].url) } else if (pins_config.pins[id].target === "same_window") { window.parent.location.href = pins_config.pins[id].url } else if (pins_config.pins[id].target === "modal") { jQuery(pins_config.pins[id].url).modal("show") } })
        }
    }
}